...
export function ForumShareButtons({ title, slug, excerpt, content, tags = [] }: ForumShareButtonsProps) {
  const [copied, setCopied] = useState(false);
  const [igCopied, setIgCopied] = useState(false);

  const getUrl = () =>
    typeof window !== "undefined"
      ? `${window.location.origin}/forums/${slug}`
      : `/forums/${slug}`;
  ...
  const payload: ContentPayload = { title, slug, excerpt, content, tags, category: "forum" };

  const shareInstagram = async () => {
    const used = await instagramChannel.share({ ...payload, imageUrl: null }, getUrl());
    if (!used) {
      setIgCopied(true);
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <button type="button" onClick={() => twitterChannel.share(payload, getUrl())} className={btnClass} aria-label="Share on X (Twitter)">
      ...
      <button type="button" onClick={() => linkedinChannel.share(payload, getUrl())} className={btnClass} aria-label="Share on LinkedIn">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      ...
      <button type="button" onClick={() => whatsappChannel.share(payload, getUrl())} className={btnClass} aria-label="Share on WhatsApp">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      ...
      <button type="button" onClick={() => threadsChannel.share(payload, getUrl())} className={btnClass} aria-label="Share on Threads">
      ...
      <button type="button" onClick={() => emailChannel.share(payload, getUrl())} className={btnClass} aria-label="Share via Email">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
