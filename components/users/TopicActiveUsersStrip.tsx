  ...
  return (
    <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Link href="/users" className="text-xs font-semibold text-sky-700 hover:underline">
          View all users
        </Link>
      </div>
      <div className="flex flex-wrap gap-3">
        {users.slice(0, 8).map((user) => (
          <Link
            key={user.id}
            href={user.last_forum_slug ? `/forums/${user.last_forum_slug}` : user.last_blog_slug ? `/blog/${user.last_blog_slug}` : "/users"}
